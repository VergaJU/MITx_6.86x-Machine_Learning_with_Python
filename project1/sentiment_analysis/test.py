...
def check_perceptron_single_update_random():
    ex_name = "Perceptron single update random"
    feature_vector= np.array([0.32210986,  0.05129646, -0.16856212, -0.24399351, -0.0903432,   0.34066892, 0.32151529, -0.16191293, -0.4275071,  0.32758594])
    label= -1
    theta= np.array([-0.10910328,  0.2704128,   0.40289849, -0.02758442,  0.38359051,  0.30383162, -0.18017209,  0.00616628, -0.18735055,  0.28056853])
    theta_0= 0.3465955196713323
    exp_res = (np.array([-0.4312131, 0.2191163, 0.5714606, 0.2164091, 0.4739337, -0.0368373, -0.5016874, 0.1680792, 0.2401566, -0.0470174]), -0.6534045)
    if check_tuple(
            ex_name + " (boundary case)", p1.perceptron_single_step_update,
            exp_res, feature_vector, label, theta, theta_0):
        return
...
def main():
    ...
    try:
        ...
        #check_perceptron_single_update_random()
        check_perceptron()
